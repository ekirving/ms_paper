...
for (page in 1:num_pages) {
    ...
    plt <- plt +
        # display as a grid
        facet_grid(description ~ ancestry, labeller = labeller(description = label_wrap_gen(width = 20))) +

        # set the axis breaks
        scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, .2), position = "left", expand = expansion(add = c(0.03, 0.03))) +
        scale_x_continuous(limits = c(xmin, xmax), breaks = xbreaks, labels = xlabels, expand = expansion(add = c(0, 390))) +
        ylab("DAF") +
        xlab("kyr BP") +
