...
p <- add_argument(p, "--out-png", help = "Output file", default = "results/compare/ancestral_paths_new-ms-r0.05-kb250-finngen-upset.png")
p <- add_argument(p, "--out-tsv", help = "Output file", default = "results/compare/ancestral_paths_new-ms-r0.05-kb250-finngen-upset.tsv")
...
if (argv$infectious) {
    ...
    top_traits <- finngen_selected %>%
        ...
        group_by(phenotype) %>%
        tally() %>%
        slice_max(order_by = n, n = as.numeric(argv$num_traits)) %>%
        pull(phenotype)
